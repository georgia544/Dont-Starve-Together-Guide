package com.dontstarvetogetherguide.app

import com.example.md5hash_crc8.Md5HashCRC8

// region Vegetables

val cornId = Md5HashCRC8.getUniqueId("corn")
val popcornId = Md5HashCRC8.getUniqueId("popcorn")

val carrotId = Md5HashCRC8.getUniqueId("carrot")
val carrotRoastedId = Md5HashCRC8.getUniqueId("carrotRoasted")

val pumpkinId = Md5HashCRC8.getUniqueId("pumpkin")
val pumpkinHotId = Md5HashCRC8.getUniqueId("pumpkinHot")

val eggplantId = Md5HashCRC8.getUniqueId("eggplant")
val eggplantBraisedId = Md5HashCRC8.getUniqueId("eggplantBraised")

val redCapId = Md5HashCRC8.getUniqueId("redCap")
val redCapCookedId = Md5HashCRC8.getUniqueId("redCapCooked")

val greenCapId = Md5HashCRC8.getUniqueId("greenCap")
val greenCapCookedId = Md5HashCRC8.getUniqueId("greenCapCooked")

val blueCapId = Md5HashCRC8.getUniqueId("blueCap")
val blueCapCookedId = Md5HashCRC8.getUniqueId("blueCapCooked")

val mandrakeId = Md5HashCRC8.getUniqueId("mandrake")
val mandrakeCookedId = Md5HashCRC8.getUniqueId("mandrakeCooked")

val petalsId = Md5HashCRC8.getUniqueId("petals")

val darkPetalsId = Md5HashCRC8.getUniqueId("darkPetals")

val lichenId = Md5HashCRC8.getUniqueId("lichen")

val glowBerryId = Md5HashCRC8.getUniqueId("glowBerry")

val foliageId = Md5HashCRC8.getUniqueId("foliage")

val cactusFleshId = Md5HashCRC8.getUniqueId("cactusFlesh")
val cactusFleshCookedId = Md5HashCRC8.getUniqueId("cactusFleshCooked")

val cactusFlowerId = Md5HashCRC8.getUniqueId("cactusFlower")

val asparagusId = Md5HashCRC8.getUniqueId("asparagus")
val asparagusCookedId = Md5HashCRC8.getUniqueId("asparagusCooked")

val tomatoId = Md5HashCRC8.getUniqueId("tomato")
val tomatoRoastedId = Md5HashCRC8.getUniqueId("tomatoRoasted")

val potatoId = Md5HashCRC8.getUniqueId("potato")
val potatoRoastedId = Md5HashCRC8.getUniqueId("potatoRoasted")

val onionId = Md5HashCRC8.getUniqueId("onion")
val onionRoastedId = Md5HashCRC8.getUniqueId("onionRoasted")

val garlicId = Md5HashCRC8.getUniqueId("garlic")
val garlicRoastedId = Md5HashCRC8.getUniqueId("garlicRoasted")

val pepperId = Md5HashCRC8.getUniqueId("pepper")
val pepperRoastedId = Md5HashCRC8.getUniqueId("pepperRoasted")

val kelpFrondsId = Md5HashCRC8.getUniqueId("kelpFronds")
val kelpFrondsCookedId = Md5HashCRC8.getUniqueId("kelpFrondsCooked")
val kelpFrondsDriedId = Md5HashCRC8.getUniqueId("kelpFrondsDried")

val stoneFruitId = Md5HashCRC8.getUniqueId("stoneFruit")
val stoneFruitCooked = Md5HashCRC8.getUniqueId("stoneFruitCooked")

val cornCodId = Md5HashCRC8.getUniqueId("cornCod")

val popperFishId = Md5HashCRC8.getUniqueId("popperFish")

val moonShroomId = Md5HashCRC8.getUniqueId("moonShroom")
val moonShroomCookedId = Md5HashCRC8.getUniqueId("moonShroomCooked")

val fireNettlesId = Md5HashCRC8.getUniqueId("fireNettles")

val tillweedId = Md5HashCRC8.getUniqueId("tillweed")

// endregion

//region Meat

val meatId = Md5HashCRC8.getUniqueId("meat")
val meatCookedId = Md5HashCRC8.getUniqueId("meatCooked")
val jerkyId = Md5HashCRC8.getUniqueId("jerky")

val monsterMeatId = Md5HashCRC8.getUniqueId("monsterMeat")
val monsterMeatCookedId = Md5HashCRC8.getUniqueId("monsterMeatCooked")
val jerkyMonsterId = Md5HashCRC8.getUniqueId("jerkyMonster")

val morselMeatId = Md5HashCRC8.getUniqueId("morselMeat")
val morselMeatCookedId = Md5HashCRC8.getUniqueId("morselMeatCooked")
val jerkySmallId = Md5HashCRC8.getUniqueId("jerkySmall")

val drumstickId = Md5HashCRC8.getUniqueId("drumstick")
val drumstickFriedId = Md5HashCRC8.getUniqueId("drumstickFried")

val fishId = Md5HashCRC8.getUniqueId("fish")
val morselFishCookedId = Md5HashCRC8.getUniqueId("morselFishCooked")

val frogLegsId = Md5HashCRC8.getUniqueId("frogLegs")
val frogLegsCookedId = Md5HashCRC8.getUniqueId("frogLegsCooked")

val batiliskWingId = Md5HashCRC8.getUniqueId("batiliskWing")
val batiliskWingCookedId = Md5HashCRC8.getUniqueId("batiliskWingCooked")

val eelId = Md5HashCRC8.getUniqueId("eel")
val eelCookedId = Md5HashCRC8.getUniqueId("eelCooked")

val koalefantTrunkId = Md5HashCRC8.getUniqueId("koalefantTrunk")
val koalefantTrunkCookedId = Md5HashCRC8.getUniqueId("koalefantTrunkCooked")

val winterKoalefantTrunkId = Md5HashCRC8.getUniqueId("WinterKoalefantTrunk")

val leafyMeatId = Md5HashCRC8.getUniqueId("leafyMeat")
val leafyMeatCookedId = Md5HashCRC8.getUniqueId("leafyMeatCooked")

val guardiansHornId = Md5HashCRC8.getUniqueId("guardiansHorn")

val deerclopsEyeballId = Md5HashCRC8.getUniqueId("deerclopsEyeball")

val fishRawId = Md5HashCRC8.getUniqueId("fishRaw")
val fishSteakCookedId = Md5HashCRC8.getUniqueId("fishSteakCooked")

val barnaclesId = Md5HashCRC8.getUniqueId("barnacles")
val barnaclesCookedId = Md5HashCRC8.getUniqueId("barnaclesCooked")

val mudfishId = Md5HashCRC8.getUniqueId("mudfish")

val deepBassId = Md5HashCRC8.getUniqueId("deepBass")

val dandyLionfishId = Md5HashCRC8.getUniqueId("dandyLionfish")

val blackCatfishId = Md5HashCRC8.getUniqueId("blackCatfish")

val dappledKoiId = Md5HashCRC8.getUniqueId("dappledKoi")

val goldenKoiId = Md5HashCRC8.getUniqueId("goldenKoi")

val wobsterId = Md5HashCRC8.getUniqueId("wobster")
val wobsterCookedId = Md5HashCRC8.getUniqueId("wobsterCooked")

val morselFishId = Md5HashCRC8.getUniqueId("morselFishId")

val runtyGruppyId = Md5HashCRC8.getUniqueId("runtyGruppy")

val bittyBaitfishId = Md5HashCRC8.getUniqueId("bittyBaitfish")

val smoltFryId = Md5HashCRC8.getUniqueId("smoltFry")

val nostrilsNakedId = Md5HashCRC8.getUniqueId("nostrilsNaked")
val nostrilsCharredId = Md5HashCRC8.getUniqueId("nostrilsCharred")

val scorchingSunfishId = Md5HashCRC8.getUniqueId("scorchingSunfish")

val bloomfinTunaId = Md5HashCRC8.getUniqueId("bloomfinTuna")

val spittlefishId = Md5HashCRC8.getUniqueId("spittleFish")

val fallounderId = Md5HashCRC8.getUniqueId("fallounder")

val sweetishFishId = Md5HashCRC8.getUniqueId("sweetishFish")

val iceBreamId = Md5HashCRC8.getUniqueId("iceBream")


//endregion

//region Fruits

val pomegranateId = Md5HashCRC8.getUniqueId("pomegranate")
val pomegranateSlicedId = Md5HashCRC8.getUniqueId("pomegranateSliced")

val dragonFruitId = Md5HashCRC8.getUniqueId("dragonFruit")
val dragonFruitCookedId = Md5HashCRC8.getUniqueId("dragonFruitCooked")

val durianId = Md5HashCRC8.getUniqueId("durian")
val durianExtraSmellyId = Md5HashCRC8.getUniqueId("durianExtraSmelly")

val berriesId = Md5HashCRC8.getUniqueId("berries")
val berriesRoastedId = Md5HashCRC8.getUniqueId("berriesRoasted")

val bananaCaveId = Md5HashCRC8.getUniqueId("bananaCave")
val bananaCaveCookedId = Md5HashCRC8.getUniqueId("bananaCaveCooked")

val watermelonId = Md5HashCRC8.getUniqueId("watermelon")
val watermelonGrilledId = Md5HashCRC8.getUniqueId("watermelonGrilled")

val juicyBerriesId = Md5HashCRC8.getUniqueId("juicyBerries")
val juicyBerriesRoastedId = Md5HashCRC8.getUniqueId("juicyBerriesRoasted")

val figId = Md5HashCRC8.getUniqueId("fig")
val figCookedId = Md5HashCRC8.getUniqueId("figCooked")

val lesserGlowBerryId = Md5HashCRC8.getUniqueId("lesserGlowBerry")


//endregion




